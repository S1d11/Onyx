(function () {
  ...
  function renderConnections() {
    ...
    // Override connected status from config (set by OAuth flows)
    connectors.forEach(c => {
      if (c.id === "github" && state.config?.githubToken) c.connected = true;
      if ((c.id === "gmail" || c.id === "gdrive") && state.config?.googleRefreshToken) c.connected = true;
    });
    ...
    connectors.forEach(conn => {
      ...
      card.addEventListener("click", () => {
        if (conn.id === "github") {
          if (!conn.connected) showGitHubConnectDialog();
          else showGitHubDisconnectDialog();
        } else if (conn.id === "gmail" || conn.id === "gdrive") {
          if (!conn.connected) showGoogleConnectDialog();
          else showGoogleDisconnectDialog();
        }
      });
  ...
  function showGitHubDisconnectDialog() {
    const existing = document.querySelector(".github-disconnect-dialog");
    if (existing) existing.remove();

    const dialog = document.createElement("div");
    dialog.className = "github-disconnect-dialog";
    ...
    dialog.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:32px;width:400px;max-width:90vw;text-align:center;">
        ...
        <h3 style="margin:0 0 8px;font-size:18px;color:var(--text);">Disconnect GitHub?</h3>
        <p style="margin:0 0 24px;font-size:13px;color:var(--text-muted);">Onyx will no longer be able to access your repositories, issues, and pull requests.</p>
        <div style="display:flex;gap:8px;justify-content:center;">
          <button id="ghDiscCancel" style="padding:10px 20px;border-radius:var(--radius-md);border:1px solid var(--border);background:var(--surface-hover);color:var(--text);font-size:14px;cursor:pointer;">Cancel</button>
          <button id="ghDiscConfirm" style="padding:10px 20px;border-radius:var(--radius-md);border:none;background:var(--danger);color:white;font-size:14px;cursor:pointer;">Sign out</button>
        </div>
      </div>
    `;
    document.body.appendChild(dialog);

    dialog.querySelector("#ghDiscCancel").addEventListener("click", () => dialog.remove());
    dialog.querySelector("#ghDiscConfirm").addEventListener("click", async () => {
      try {
        await call("disconnectGitHub", {});
        state.config.githubToken = null;
        dialog.remove();
        renderConnections();
        toast("GitHub disconnected");
      } catch (e) {
        toast("Error disconnecting: " + (e.message || e));
      }
    });
  }

  function showGoogleDisconnectDialog() {
    const existing = document.querySelector(".google-disconnect-dialog");
    if (existing) existing.remove();

    const dialog = document.createElement("div");
    dialog.className = "google-disconnect-dialog";
    dialog.style.cssText = "position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;z-index:300;";
    dialog.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:32px;width:400px;max-width:90vw;text-align:center;">
        ...
        <h3 style="margin:0 0 8px;font-size:18px;color:var(--text);">Disconnect Google?</h3>
        <p style="margin:0 0 24px;font-size:13px;color:var(--text-muted);">Onyx will no longer be able to access your Gmail and Google Drive.</p>
        <div style="display:flex;gap:8px;justify-content:center;">
          <button id="gDiscCancel" style="padding:10px 20px;border-radius:var(--radius-md);border:1px solid var(--border);background:var(--surface-hover);color:var(--text);font-size:14px;cursor:pointer;">Cancel</button>
          <button id="gDiscConfirm" style="padding:10px 20px;border-radius:var(--radius-md);border:none;background:var(--danger);color:white;font-size:14px;cursor:pointer;">Sign out</button>
        </div>
      </div>
    `;
    document.body.appendChild(dialog);

    dialog.querySelector("#gDiscCancel").addEventListener("click", () => dialog.remove());
    dialog.querySelector("#gDiscConfirm").addEventListener("click", async () => {
      try {
        await call("disconnectGoogle", {});
        state.config.googleRefreshToken = null;
        dialog.remove();
        renderConnections();
